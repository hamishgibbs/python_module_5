...
Import csv data from `URL HERE` using `pandas.read_csv()`.
...
Notice that the column indices are no longer sequential. This is because the order of the rows in the dataframe has changed, but the row indices have not been changed. This is an important feature, as indices can be used to referenced specific rows through different processing steps. 
...
Use `pandas.DataFrame.dropna` to drop rows with a missing `age` value. Use `axis` argument to specify that rows should be dropped, not columns. Use the `subset` argument to drop records with a null value in the `age` column, not in all columns. Use the argument `inplace = True` to alter the original dataset, not a copy
...
Imputation refers to "filling in" missing values with numeric values derived from the dataset. **Be very careful** when imputing values during data cleaning - the wrong approach to imputation can introduce bias in subsequent analyses. 
...
Regular expressions (regex) are flexible, reusable expressions for matching or extracting data from a string. regex in Python can be considered as a small programming language independent from Python, with unique syntax and a limited application. 
